...
.dummy/
  memory/
    ...
    user/
      ...
      profile.md       — quem é o usuário, objetivos, contexto (opcional)
    global/
      errors.md        — erros corrigidos globalmente (reutilizáveis entre projetos)
      patterns.md      — padrões que funcionaram bem (opcional)
      execution-log.md — log de execução de skills por sessão (observabilidade)
      dream-log.md     — histórico de consolidações de memória
```
...
### app-factory-multiagent → state.md
...
| Arquivo criado com sucesso | app-factory-multiagent | state.md |
...
| Execução de skill concluída (sucesso/falha) | qualquer skill | global/execution-log.md |
| Sessão encerrando ("bye dummy") | skill4d-core-orchestrator | state.md |
